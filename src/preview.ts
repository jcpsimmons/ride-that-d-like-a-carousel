...
let currentConfig: CarouselConfig = JSON.parse(
  ...
  fetch(req) {
    ...
    if (url.pathname === "/") {
      ...
      const nav = Array.from(
        { length: currentConfig.slides.length },
        (_, i) => `<a href="/slide/${i}">Slide ${i + 1}</a>`,
      ).join(" | ");
      return new Response(
        `
...
console.log(
  "Live reload enabled. Edit your files and the browser will update.",
);
